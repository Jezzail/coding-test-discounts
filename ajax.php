<?php
	...
	function discountCustomer($json_order, $json_customer){
		...
		if($revenue > 1000){//if its over 1000, 10% discount
			$descuento = $json_order['total_total'] * (10 / 100);
			$descuento = round($descuento, 2, PHP_ROUND_HALF_DOWN);
			$json_order['discount_1']=$descuento;
			$json_order['total_discounts'] += $descuento;
			$json_order['total_total'] -= $descuento;
		}
	...
	function discountSwitches($json_order, $json_product){
		foreach($json_order['items'] as $key => $val){
			...
			if($category==2 && $val['quantity']>5){
				$multiplo = floor($val['quantity']/6);
				$descuento = $multiplo*$val['unit-price'];
				$json_order['items'][$key]['discount_2']=$descuento;
				$json_order['total_discounts'] += $descuento;
				$json_order['total_total'] -= $descuento;
			}
		}
	...
	function discountTools($json_order, $json_product){
		...
		//20% discount
		$descuento = $json_order['items'][$first_key]['total'] * (20 / 100);
		$descuento = round($descuento, 2, PHP_ROUND_HALF_DOWN);
		$json_order['items'][$first_key]['discount_3']=$descuento;
		$json_order['total_discounts'] += $descuento;
		$json_order['total_total'] -= $descuento;
